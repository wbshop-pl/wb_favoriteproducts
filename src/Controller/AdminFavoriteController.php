...
namespace WbShop\WbFavoriteProducts\Controller;

use WbShop\WbFavoriteProducts\Grid\Filter\FavoriteListFilters;
use WbShop\WbFavoriteProducts\Provider\DateFiltersProvider;
...
class AdminFavoriteController extends FrameworkBundleAdminController
{
    ...
    {
        $favoriteGridFactory = $this->get('wbshop.wb_favoriteproducts.grid.favorite_list_grid_factory');
        ...
        return $this->render('@Modules/wb_favoriteproducts/views/templates/admin/grid/index.html.twig', [
            'grid' => $this->presentGrid($favoriteGrid),
            'tabs' => $this->buildDateTabs($request),
            'title' => $this->trans('Favorite products analytics', 'Modules.Wbfavoriteproducts.Admin'),
        ]);
    }
    ...
    private function buildDateTabs(Request $request): array
    {
        ...
        foreach ($dateFiltersProvider->getDateFilters() as $filter) {
            $tabs[] = [
                ...
                'url' => $this->generateUrl('wb_favoriteproducts_controller_index', [
                    'date_range' => $filter,
                ]),
    ...
    private function getTitleByDateRange(string $filterName): string
    {
        ...
        switch ($filterName) {
            case DateFiltersProvider::ALL_TIME:
                $title = $this->trans('All time', 'Modules.Wbfavoriteproducts.Admin');
                break;
            case DateFiltersProvider::LAST_24_HOURS:
                $title = $this->trans('Last 24 hours', 'Modules.Wbfavoriteproducts.Admin');
                break;
            case DateFiltersProvider::LAST_3_DAYS:
                $title = $this->trans('Last 3 days', 'Modules.Wbfavoriteproducts.Admin');
                break;
            case DateFiltersProvider::LAST_7_DAYS:
                $title = $this->trans('Last 7 days', 'Modules.Wbfavoriteproducts.Admin');
                break;
            case DateFiltersProvider::LAST_30_DAYS:
                $title = $this->trans('Last 30 days', 'Modules.Wbfavoriteproducts.Admin');
                break;
            case DateFiltersProvider::LAST_90_DAYS:
                $title = $this->trans('Last 90 days', 'Modules.Wbfavoriteproducts.Admin');
                break;
        }
